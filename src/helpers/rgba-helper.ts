...
const removeHash = (hex: string) => (hex.startsWith('#') ? hex.slice(1) : hex);
 ...
 */
const hexToRgba = (hex: ColorValue | string, a: number) => {
  const hashlessHex = removeHash(hex as string);
...
/**
 * Turns an old-fashioned css rgb color value into a rgb color value.
 *
 * If you specify an alpha value, you'll get a rgba() value instead.
 *
 * @param rgb - The rgb value to convert. ('rgb(11, 22, 33)')
 * @param a - An alpha value to apply. (optional) ('0.5', '0.25')
 * @return An rgb or rgba value. ('rgb(11, 22, 33)'. 'rgba(11, 22, 33, 0.5)')
 */
